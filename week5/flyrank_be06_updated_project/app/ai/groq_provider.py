        ...
        for attempt in range(attempts):
            try:
                ...
                if attempt < self.max_retries:
                    ...
                    continue
                raise AIProviderTimeout("The AI provider timed out.", retryable=True) from exc
            except httpx.RequestError as exc:
                if attempt < self.max_retries:
                    await self._backoff(attempt)
                    continue
                raise AIProviderError(
                    "The AI provider could not be reached.", retryable=True
                ) from exc
            ...
            logger.info(
                "ai_call feature=%s provider=groq model=%s input_tokens=%d "
                "output_tokens=%d estimated_cost_usd=%.8f",
                feature,
                self.model,
                input_tokens,
                output_tokens,
