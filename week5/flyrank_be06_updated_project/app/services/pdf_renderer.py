...
def render_sales_report(
    ...
    overview.setStyle(_standard_table_style())
    ...
    status_data.extend(
        [[m.status.title(), m.order_count, _money(m.revenue)] for m in summary.by_status]
    )
    ...
    daily_data.extend([[m.date, m.order_count, _money(m.revenue)] for m in summary.daily])
    if len(daily_data) == 1:
